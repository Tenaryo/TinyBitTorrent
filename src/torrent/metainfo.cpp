...
auto from_info_dict(const bencode::Dict& info_dict) -> Metainfo {
    ...
    for (const auto& [key, val] : info_dict.items_) {
        if (key == "length") {
            length = std::get<bencode::Integer>(val);
            found_length = true;
        } else if (key == "piece length") {
    ...
    if (!found_length) {
        throw std::runtime_error("missing info.length in metadata");
    }
    if (!found_piece_length) {
        throw std::runtime_error("missing info.piece length in metadata");
    }
    if (!found_pieces) {
        throw std::runtime_error("missing info.pieces in metadata");
    }

    util::Sha1 hasher;
    hasher.update(bencode::encode(bencode::Value{info_dict}));
    auto digest = hasher.finalize();
    std::string info_hash(reinterpret_cast<const char*>(digest.data()),
                          digest.size());
    return {"",
